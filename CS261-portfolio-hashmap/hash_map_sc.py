...
# Assignment: 6
# Due Date: 12/5/24
# Description: A separate chaining HashMap implementation
...
class HashMap:
    ...
    def put(self, key: str, value: object) -> None:
        ...
        if node:
            ...
            # Insert a new key-value pair if key does not exist
    ...
    def resize_table(self, new_capacity: int) -> None:
        ...
        """
        # Ignore if the new capacity is less than 1
        if new_capacity < 1:
            return

        new_capacity = self._next_prime(new_capacity)

        # Save old buckets and initialize new buckets
        ...
        # Reinsert all key-value pairs into the new hash map
...
def find_mode(da: DynamicArray) -> tuple[DynamicArray, int]:
    """
    Returns a tuple containing the most occurring
    values of the array and the number of occurrences
    """
    ...
    # Create a DynamicArray to store the mode
    modes = DynamicArray()

    # Identify all values with the maximum frequency
    for i in range(da.length()):
        value = da[i]
        if map.get(value) == max_freq:
            # Add the mode only once
            map.put(value, 0)  # Avoid duplicate checks by resetting frequency
            modes.append(value)
